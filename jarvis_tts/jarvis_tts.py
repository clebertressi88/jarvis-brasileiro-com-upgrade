...
class JarvisTTS:
    def __init__(
        ...
        self.model = TTSModel.load_model(language="portuguese")
        ...
        try:
            ...
            if "voice cloning" not in str(exc).lower():
                raise
            logger.warning(
                "Voice cloning weights are unavailable; using the built-in "
                "Portuguese 'rafael' voice instead."
            )
            self.voice_state = self.model.get_state_for_audio_prompt("rafael")
        ...
        while not self._stop_event.is_set():
            ...
            if now >= next_send:
                ...
                if block is not None:
                    ...
                    # Send one silent frame so the UI can settle, then stop
                    # flooding WebView with ~100 empty frames per second.
                    if silence_sent:
                        next_send = now + 0.05
                        time.sleep(0.05)
                        continue
